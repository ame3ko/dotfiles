...
### 現在の接続状況（このMacBook・検証済み: 2026-07-04）

`scripts/session-start-check.sh` で全5リンクの張り直し不要（＝正しくリンク済み）を確認済み。
...
graph LR
    ...
    subgraph CURSOR_DIR["Cursor"]
        A2["~/.cursor/rules"]
        C1[".../Cursor/User/settings.json"]
    end

    subgraph VSCODE_DIR["VS Code"]
        D1[".../Code/User/settings.json"]
    end
...
```mermaid
graph LR
    subgraph repo["~/dotfiles（実体・編集はここだけ）"]
        A["claude/CLAUDE.md"]
        B["claude/settings.json"]
        C["cursor/settings.json"]
        D["vscode/settings.json"]
    ...
    subgraph cursor_dir["Cursor 設定"]
        A2["~/.cursor/rules"]
        C1[".../Cursor/User/settings.json"]
    end

    subgraph vscode_dir["VS Code 設定"]
        D1[".../Code/User/settings.json"]
    end
...
```mermaid
flowchart TD
    E["設定ファイルを編集\n(dotfiles 内)"] --> C["git add / commit"]
    C --> P["git push"]
    P --> R[("GitHub リポジトリ")]
    R --> Pull["他端末で git pull"]
    Pull --> Sync["シンボリックリンク経由で\n各ツールに即反映"]
    Sync -.変更したくなったら再度.-> E
```
...
## Cursor を新規インストールした場合（別端末など）

新しい端末では `cursor/settings.json` は空の状態から始まる想定。インストール後に以下を実施する。
